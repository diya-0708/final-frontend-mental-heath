...
export const changePassword = async (role, currentPassword, newPassword, confirmPassword) => {
  ...
  // ✅ Use the correct axios instance based on role
  const instance = role === 'admin' ? adminApi : api
...
export const addAdmin = async (email, password) => {
  // ✅ Use adminApi — admin token is required for this endpoint
  const response = await adminApi.post('/admin/add-admin', {
    email,
    password,
  })
  return response.data
