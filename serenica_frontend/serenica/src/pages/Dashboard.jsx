...
function RiskBadge({ level }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 text-xs font-medium font-body px-3 py-1.5 rounded-full border ${cfg.color}`}
    >
      <Icon size={12} />
      {level}
    </span>
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (data.length > 0) {
          // Key combines user id + login timestamp so it resets on every new login.
          // AuthContext must call: sessionStorage.setItem("serenica_login_ts", Date.now())
          // inside the login() function right after setting the token.
          const loginTs = sessionStorage.getItem("serenica_login_ts") || "0";
          const shownKey = `welcome_shown_${user?.id}_${loginTs}`;
  ...
  return (
    <>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
          ...
          >
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 text-sm font-body font-medium text-teal-700 border border-teal-200 bg-white hover:bg-teal-50 rounded-xl transition-all shadow-sm mt-1"
            >
              <KeyRound size={14} />
      ...
      {showChangePassword &&
        createPortal(
          <ChangePasswordModal role="student" onClose={() => setShowChangePassword(false)} />,
          document.body
        )}
